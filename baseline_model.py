...
all_datas = readFile('Hinglish_train_14k_split_conll.txt')
for i in range(10):
    print(all_datas[i].content, "\n", all_datas[i].sentiment, "\n", all_datas[i].uid)
    print("*"*100)
...
with open('Hinglish_test_labels.txt','r') as fp:
    line = fp.readline()
    while True:
        line = fp.readline()
        if not line:
...
all_test_tweets = readFile('Hinglish_test_unalbelled_conll_updated.txt',test_data=True)
